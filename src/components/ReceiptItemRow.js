export default function ReceiptItemRow({ item, name, quantity, unitPrice }) {
  ...
  return (
    <div className="flex items-start justify-between py-1.5 px-2 border-b border-border-light last:border-b-0">
      ...
      <p className="text-text-primary font-semibold font-mono text-sm">RWF {Number(lineTotal).toFixed(0)}</p>
    </div>
  );
} 